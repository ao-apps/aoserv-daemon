 ...
 */
final public class HttpdManagerMandrivaRedHat extends BuilderThread {
    ...
    private static HttpdManagerMandrivaRedHat httpdManagerMandrivaRedHat;

    private HttpdManagerMandrivaRedHat() {
    }
    ...
    private static final void copyResource(String resource, OutputStream out) throws IOException {
        InputStream in=HttpdManagerMandrivaRedHat.class.getResourceAsStream(resource);
        try {
    ...
    private static void doRebuildHttpdSites(
        ...
        if(httpdLSA==null) throw new SQLException("Unable to find LinuxServerAccount: "+httpdLA+" on "+aoServer.getHostname());
        int httpdUID=httpdLSA.getUID().getID();
        LinuxServerAccount awstatsLSA=aoServer.getLinuxServerAccount(LinuxAccount.AWSTATS);
        if(awstatsLSA==null) throw new SQLException("Unable to find LinuxServerAccount: "+LinuxAccount.AWSTATS+" on "+aoServer.getHostname());
    ...
    private static void writeHttpdSiteBindFile(HttpdSiteBind bind, UnixFile file, String siteInclude) throws IOException, SQLException {
        ...
        int osv=netBind.getServer().getOperatingSystemVersion().getPkey();
        String ipAddress = netBind.getIPAddress().getIPAddress();
    ...
    /**
     * Should be called by HttpdManager for Mandriva and RedHat only.
     */
    static void start() throws IOException, SQLException {
        synchronized(System.out) {
            if(
                httpdManagerMandrivaRedHat==null
            ) {
                System.out.print("Starting HttpdManager: ");
                AOServConnector connector=AOServDaemon.getConnector();
                httpdManagerMandrivaRedHat=new HttpdManagerMandrivaRedHat();
                connector.httpdBinds.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdServers.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdJBossSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdSharedTomcats.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdSiteAuthenticatedLocationTable.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdSiteBinds.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdSiteURLs.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdStaticSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatContexts.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatDataSources.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatParameters.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatSharedSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdTomcatStdSites.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.httpdWorkers.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.ipAddresses.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.linuxServerAccounts.addTableListener(httpdManagerMandrivaRedHat, 0);
                connector.netBinds.addTableListener(httpdManagerMandrivaRedHat, 0);
    ...
    public static void waitForRebuild() {
        if(httpdManagerMandrivaRedHat!=null) httpdManagerMandrivaRedHat.waitForBuild();
    }
    ...
    public static String getAutoWarning(HttpdSite hs) throws IOException, SQLException {
        return
            ...
            + "\n"
            + "  AOSH: "+AOSHCommand.SET_HTTPD_SITE_IS_MANUAL+" "+hs.getSiteName()+' '+hs.getAOServer().getHostname()+" true\n"
            + "\n"
    ...
    public static String getAutoWarning(HttpdSharedTomcat hst) throws IOException, SQLException {
        return
            ...
            + "\n"
            + "  AOSH: "+AOSHCommand.SET_HTTPD_SHARED_TOMCAT_IS_MANUAL+" "+hst.getName()+' '+hst.getAOServer().getHostname()+" true\n"
            + "\n"
            + "  support@aoindustries.com\n"
            + "  (866) 270-6195\n"
            + "-->\n"
