...
/**
 * Manages SharedTomcat version 5.5.X configurations.
 *
...
class HttpdSharedTomcatManager_5_5_X extends HttpdSharedTomcatManager<TomcatCommon_5_5_X> {
	...
	HttpdSharedTomcatManager_5_5_X(SharedTomcat sharedTomcat) {
		super(sharedTomcat);
	}
	...
	void buildSharedTomcatDirectory(String optSlash, PosixFile sharedTomcatDirectory, List<File> deleteFileList, Set<SharedTomcat> sharedTomcatsNeedingRestarted) throws IOException, SQLException {
		...
		if (!sharedTomcatStat.exists() || sharedTomcatStat.getUid() == PosixFile.ROOT_GID) {
			...
			new PosixFile("/opt/apache-tomcat-5.5/conf/tomcat-users.xml").copyTo(tuUF, false);
			tuUF.chown(lsaUID, lsgGID).setMode(0660);

			// Create the web.xml file.
			PosixFile webUF=new PosixFile(wwwGroupDir+"/conf/web.xml");
			new PosixFile("/opt/apache-tomcat-5.5/conf/web.xml").copyTo(webUF, false);
