    ...
    try {
      ...
      if (incomingDirectory.getStat().exists()) {
        ...
        while (!Thread.currentThread().isInterrupted()) {
          ...
          if (incomingDirectoryList == null || incomingDirectoryList.length == 0) {
            break;
          }
                ...
                for (String userFilename : userDirectoryList) {
                  ...
                  if (userFilename.startsWith("ham_") || userFilename.startsWith("spam_")) {
                    ...
                    if (userUfStat.isRegularFile()) {
                      ...
                      if (pos1 == -1) {
                        throw new AssertionError("pos1 == -1: This should not happen because of check against ham_ or spam_ above.");
                      }
          ...
          for (int c = 1; c < readyList.size(); c++) {
            ...
            if (firstIsHam == otherIsHam) {
              ...
              if (thisPass.size() >= MAX_SALEARN_BATCH) {
                break;
              }
            } else {
        ...
        try {
            ...
            try (ChainWriter newOut = new ChainWriter(bout)) {
              ...
              if (spamdInetAddress != null) {
                ...
                if (spamdInetAddress.isLoopback()) {
                  ...
                  if (usedIps.isEmpty()) {
                    throw new AssertionError("No ip addresses in the same family as " + spamdInetAddress);
                  }
                }
              }
              newOut.print("\"\n");
          ...
          for (UserServer lsa : lsas) {
            ...
            if (!integrationMode.getName().equals(SpamAssassinMode.NONE)) {
              ...
              if (spamdBind == null) {
                throw new SQLException("Account has SpamAssassin integration enabled, but SpamAssassin is not configured in net_binds: " + lsa);
              }
