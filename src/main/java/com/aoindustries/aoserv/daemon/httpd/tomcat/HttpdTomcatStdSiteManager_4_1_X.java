 ...
 * Copyright (C) 2007-2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2025, 2026  AO Industries, Inc.
...
class HttpdTomcatStdSiteManager_4_1_X extends HttpdTomcatStdSiteManager<TomcatCommon_4_1_X> {
  ...
  protected void buildSiteDirectoryContents(String optSlash, PosixFile siteDirectory, boolean isUpgrade) throws IOException, SQLException {
    ...
    try (
        ChainWriter out = new ChainWriter(
            new BufferedOutputStream(
                new PosixFile(siteDir + "/bin/tomcat").getSecureOutputStream(uid, gid, 0700, false, uidMin, gidMin)
            )
        )
        ) {
      out.print("#!/bin/sh\n"
     ...
     */
    final PosixFile tu = new PosixFile(siteDir + "/conf/tomcat-users.xml");
    new PosixFile("/opt/apache-tomcat-4.1/conf/tomcat-users.xml").copyTo(tu, false);
    tu.chown(uid, gid).setMode(0660);

    final PosixFile wx = new PosixFile(siteDir + "/conf/web.xml");
    new PosixFile("/opt/apache-tomcat-4.1/conf/web.xml").copyTo(wx, false);
