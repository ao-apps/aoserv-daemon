                ...
                try (ChainWriter out = new ChainWriter(bout)) {
                  if (
                    ...
                    for (Database db : pds) {
                      ...
                      if (!didOne) {
                        throw new SQLException("No users for database " + db);
                      }
                    ...
                    for (Database db : pds) {
                      ...
                      if (
                        ...
                        if (dbUsers.isEmpty()) {
                          throw new SQLException("No users found for database (should always have at least datdba): " + db);
                        }
                        if (isLocalhost) {
                          if (!didComment) {
                            out.print('\n');
                ...
                if (needsReload) {
                  ...
                  if (pidFile.exists()) {
                    ...
                    for (int d = 0; d < pid.length(); d++) {
                      ...
                      if (ch < '0' || ch > '9') {
                        throw new IOException("Invalid character in postmaster.pid first line: " + ch);
                      }
                    }
