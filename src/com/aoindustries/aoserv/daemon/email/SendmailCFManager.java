...
final public class SendmailCFManager extends BuilderThread {
    ...
    private static final UnixFile
        mc=new UnixFile("/etc/mail/sendmail.mc"),
        newMC=new UnixFile("/etc/mail/sendmail.mc.new"),
        cf=new UnixFile("/etc/mail/sendmail.cf"),
        cfNew=new UnixFile("/etc/mail/sendmail.cf.new")
    ;
    ...
    protected void doRebuild() throws IOException, SQLException {
        ...
        synchronized(rebuildLock) {
            ...
            if(!mc.getStat(tempStat).exists() || !mc.contentEquals(newBytes)) {
                FileOutputStream fout = newMC.getSecureOutputStream(UnixFile.ROOT_UID, UnixFile.ROOT_GID, 0644, true);
                try {
                    fout.write(newBytes);
                } finally {
                    fout.close();
            ...
            // Rebuild the /etc/sendmail.cf file if modified time is before sendmail.mc or doesn't exist
            Stat cfStat = cf.getStat();
            if(
                !cfStat.exists()
                || cfStat.getModifyTime()<mc.getStat(tempStat).getModifyTime()
            ) {
                String[] command;
                if(
                    osv==OperatingSystemVersion.MANDRIVA_2006_0_I586
            ...
            // TODO: Rebuild submit.mc as needed, too.  Restart sendmail only once in this case.
        }
    }
