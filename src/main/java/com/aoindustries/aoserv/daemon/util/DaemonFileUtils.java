 ...
 * Copyright (C) 2008, 2009, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024  AO Industries, Inc.
...
public final class DaemonFileUtils {
  ...
  public static void createEmptyFile(PosixFile uf, int mode, int uid, int gid) throws IOException {
    ...
    if (!ufStat.exists()) {
      ...
      throw new IOException("File exists and is not a regular file: " + uf.getPath());
    }
    if (ufStat.getMode() != mode) {
      uf.setMode(mode);
    }
    ...
    if (
      ...
      try (TempFileContext tempFileContext = new TempFileContext(file.getFile().getParentFile())) {
        ...
        if (backupFile != null && fileStat.exists()) {
          ...
          if (logger.isLoggable(Level.FINE)) {
            logger.fine("cp \"" + file + "\" \"" + backupTemp + "\", " + numBytes + " bytes copied");
          }
          if (fileStat.getSize() != numBytes) {
            throw new IOException("File size mismatch: " + fileStat.getSize() + " != " + numBytes);
          }
          Stat backupTempStat = backupTemp.getStat();
          if (backupTempStat.getSize() != numBytes) {
            throw new IOException("File size mismatch: " + backupTempStat.getSize() + " != " + numBytes);
          }
          // Set ownership
          if (backupTempStat.getUid() != uid || backupTempStat.getGid() != gid) {
            if (logger.isLoggable(Level.FINE)) {
              logger.fine("chown " + uid + ':' + gid + " \"" + backupTemp + '"');
            }
          ...
          if (backupTempStat.getMode() != mode) {
            if (logger.isLoggable(Level.FINE)) {
              logger.fine("chmod " + Long.toOctalString(mode) + " \"" + backupTemp + '"');
            }
        ...
        if (logger.isLoggable(Level.FINE)) {
          logger.fine("Wrote " + newContents.length + " bytes to \"" + fileTemp + '"');
        }
        Stat fileTempStat = fileTemp.getStat();
        if (fileTempStat.getSize() != newContents.length) {
          throw new IOException("File size mismatch: " + fileTempStat.getSize() + " != " + newContents.length);
        }
        // Set ownership
        if (fileTempStat.getUid() != uid || fileTempStat.getGid() != gid) {
          if (logger.isLoggable(Level.FINE)) {
            logger.fine("chown " + uid + ':' + gid + " \"" + fileTemp + '"');
          }
        ...
        if (fileTempStat.getMode() != mode) {
          if (logger.isLoggable(Level.FINE)) {
            logger.fine("chmod " + Long.toOctalString(mode) + " \"" + fileTemp + '"');
          }
        ...
        if (backupTemp != null) {
          if (logger.isLoggable(Level.FINE)) {
            logger.fine("mv \"" + backupTemp + "\" \"" + backupFile + '"');
          }
          backupTemp.renameTo(backupFile);
          if (restorecon != null) {
            restorecon.add(backupFile);
          }
        }
        // Move file into place
        if (logger.isLoggable(Level.FINE)) {
          logger.fine("mv \"" + fileTemp + "\" \"" + file + '"');
        }
      ...
      if (fileStat.getUid() != uid || fileStat.getGid() != gid) {
        if (logger.isLoggable(Level.FINE)) {
          logger.fine("chown " + uid + ':' + gid + " \"" + file + '"');
        }
        file.chown(uid, gid);
        updated = true;
      }
      ...
      if (fileStat.getMode() != mode) {
        if (logger.isLoggable(Level.FINE)) {
          logger.fine("chmod " + Long.toOctalString(mode) + " \"" + file + '"');
        }
        file.setMode(mode);
        updated = true;
      }
