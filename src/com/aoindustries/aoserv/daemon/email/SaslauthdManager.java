		...
		try {
			...
			synchronized(rebuildLock) {
				...
				if(PackageManager.getInstalledPackage(PackageManager.PackageName.CYRUS_SASL) != null) {
					...
					try {
						// When there is at least one linux account with '@' in the username, must add -r switch to saslauthd
						boolean hasAtUser = false;
						for(LinuxServerAccount lsa : thisAoServer.getLinuxServerAccounts()) {
							if(lsa.getLinuxAccount().getUsername().getUsername().toString().indexOf('@') != -1) {
								hasAtUser = true;
								break;
							}
						}
						if(logger.isLoggable(Level.FINE)) {
							logger.fine(
								hasAtUser
									? "Has at least one user with '@' in the username, -r will be provided to saslauthd"
									: "No user with '@' in the username, -r will not be provided to saslauthd"
							);
						}
